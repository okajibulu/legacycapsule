...
// Called by PaymentService only — do not call directly from API routes.
// ─────────────────────────────────────────────────────────────────────────────
...
// Stripe does not multiply these by 100 — amount IS the integer sent to Stripe.
...
export interface RegionalPrice {
  pricing_key:      string
  amount:           number   // human-readable (e.g. 50 for €50, 40000 for ₦40,000)
...
export async function getRegionalPrice(
  ...
  if (zone.is_independent) {
    ...
    // All other zones: EUR base × zone multiplier, rounded to nearest integer
  ...
  // Stripe amount: zero-decimal currencies send amount as-is, others × 100
