// ─────────────────────────────────────────────────────────────────────────────
// FILE PATH: app/api/contributions/reject/route.ts
// PURPOSE:   Rejects (deletes) a pending contribution from the organiser
//            manage dashboard. Replaces the direct Supabase client call in
//            manage/[slug]/page.tsx handleDecline.
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (deleteError) {
      console.error('[contributions/reject] Delete error:', deleteError)
      return NextResponse.json(
    ...
    await logAction({
      ...
      action_key:   ACTION_KEYS.CONTRIBUTION_REJECTED,
      action_label: `Declined a voice from ${contribution.contributor_name}`,
    ...
    console.error('[contributions/reject]', err)
    return NextResponse.json(
      { error: 'Something went wrong. Please try again.' },
      { status: 500 }
    )
