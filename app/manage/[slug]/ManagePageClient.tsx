...
// ============================================================
// FILE PATH: app/manage/[slug]/ManagePageClient.tsx
// NOTE:      This file was app/manage/[slug]/page.tsx until AI21v2.12.15.
//            The server component (page.tsx) now handles auth and passes
//            accountType, accountName, accountEmail, permissions as props.
...
import ContributorGalleryManager from '@/components/manage/ContributorGalleryManager'
...
function FamilyRepElderSection({ capsuleId, capsuleSlug, honoureeName }: {
  ...
  const fetchElders = async () => {
    try {
      // ── Option A silent migration — runs once, idempotent ──────────────
      // If capsule has legacy family_rep_email, silently create Elder account
      await fetch('/api/team/migrate-family-rep', {
        method:  'POST',
        headers: { 'Content-Type': 'application/json' },
        body:    JSON.stringify({ capsule_id: capsuleId, capsule_slug: capsuleSlug }),
      }).catch(() => {}) // non-blocking — never fails the fetch
  ...
  return (
    <div>
      <p style={{ fontSize: '12px', color: textFaint, lineHeight: 1.7, marginBottom: '14px' }}>
        Family Representatives receive a private portal link to view and respond to all voices, acknowledgements, and set the Family Appreciation message on behalf of the family. You can have multiple Family Representatives — each gets their own personal link.
      </p>
      {elders.map(elder => (
        <FamilyRepElderCard
          key={elder.id}
...
/* -- BOTTOM NAV ----------------------------------------- */
function BottomNav({ active, onChange, pendingCount, isCoadmin, permissions }: {
  active: Tab; onChange: (t: Tab) => void; pendingCount: number
  isCoadmin?: boolean; permissions?: string[]
}) {
  // Co-admin sees only tabs for their granted permissions
  // Overview always shown — it's the home screen
  const canSeeTab = (tab: Tab): boolean => {
    if (!isCoadmin) return true
    if (tab === 'overview') return true
    if (tab === 'settings') return true
    if (tab === 'setstories' && permissions?.includes('stories')) return true
    if (tab === 'setprofile') return true  // profile canvas is view-only for all
    if (tab === 'services') return false   // co-admins don't see services/billing
    return false
  }
  ...
  return (
    <div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, zIndex: 50, background: 'rgba(15,10,30,0.98)', backdropFilter: 'blur(20px)', borderTop: `1px solid rgba(226,195,107,0.15)`, display: 'flex', padding: '6px 8px max(8px, env(safe-area-inset-bottom))' }}>
      {tabs.filter(tab => canSeeTab(tab.id)).map((tab: { id: Tab; label: string; icon: string }) => (
...
interface ManagePageClientProps {
  slug:                string
  serverAccountType:   'organiser' | 'family_rep_full_access' | 'coadmin'
  serverAccountName:   string | null
  serverAccountEmail:  string | null
...
export default function ManagePageClient({
  ...
  const [visitorEmail,   setVisitorEmail]   = useState(serverAccountEmail ?? '')
  const [accountType,    setAccountType]    = useState(serverAccountType)
  const [accountName,    setAccountName]    = useState(serverAccountName ?? '')
  const [permissions,    setPermissions]    = useState<string[]>(serverPermissions)
...
if (storiesRes.data) setStories(storiesRes.data.map((s: any) => ({
  ...
  // FRFA has full access — treated as organiser for dashboard rendering
  const isOrganiser = (
    (visitorEmail !== '' && visitorEmail.toLowerCase() === capsule?.organiser_email?.toLowerCase()) ||
    accountType === 'family_rep_full_access'
  )
  // Co-admin: access scoped to their granted permissions
      ...
      <div style={{ minHeight: '100vh', background: bg, fontFamily: "'DM Sans', sans-serif", color: textPrimary, paddingBottom: '80px' }}>
        ...
        {accountType !== 'organiser' && (
              <span style={{
                ...
                background: accountType === 'family_rep_full_access'
                  ? 'rgba(134,239,172,0.08)'
                  : 'rgba(196,181,253,0.08)',
                border: `1px solid ${accountType === 'family_rep_full_access'
                  ? 'rgba(134,239,172,0.2)'
                  : 'rgba(196,181,253,0.2)'}`,
                color: accountType === 'family_rep_full_access'
                  ? 'rgba(134,239,172,0.8)'
                  : 'rgba(196,181,253,0.8)',
                letterSpacing: '0.08em', textTransform: 'uppercase',
              }}>
                {accountType === 'family_rep_full_access' ? 'FR Full Access' : 'Co-admin'}
              </span>
            )}
              </div>
        </div>
              ...
              {settingsTab === 'capsule' && (
                ...
                {/* ── My Account section ── */}
                  <AccountDetailsSection
                    ...
                    accountName={accountName || visitorEmail}
