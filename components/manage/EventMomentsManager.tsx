...
//            Actions: show/hide, reorder, feature for publication,
//            upload official photography.
//            Accessed from manage dashboard S/Stories or dedicated tab.
...
// ═══ SECTION 2 — Phase selector sub-component ═══
...
// ═══ SECTION 3 — Photo card sub-component ═══
        ...
        <div style={{
          ...
          {photo.featured_in_publication && (
            <span style={{
              padding:      '2px 6px',
              borderRadius: '6px',
              background:   'rgba(226,195,107,0.85)',
              fontSize:     '9px',
              fontWeight:   700,
              color:        '#1a0845',
            }}>
...
export default function EventMomentsManager({
  ...
  const fetchPhotos = useCallback(async () => {
    ...
    try {
      ...
      if (data.ok) {
        // Merge guest + official into one manageable list
        const all: GalleryPhoto[] = [
          ...(data.guest_photos    ?? []).map((p: any) => ({ ...p, approved: true,  is_official_photography: false })),
          ...(data.official_photos ?? []).map((p: any) => ({ ...p, approved: true,  is_official_photography: true  })),
        ]
        // Re-fetch full records with approved + featured status
        await fetchFullPhotos()
      }
    } catch (e) {
      console.error('[EventMomentsManager] fetch error:', e)
    } finally {
      setLoading(false)
    }
  }, [activePhaseId])

  // ── Fetch full photo records with all fields ───────────────────
    ...
    <div style={{ padding: '20px 0' }}>
      ...
      <div style={{
        ...
        <p style={{
          ...
          Add formal photographer shots for this moment. These appear in a separate section.
        </p>
        ...
        {uploadError && (
          <p style={{ margin: '8px 0 0', fontSize: '11px', color: 'rgba(248,113,113,0.8)' }}>
            {uploadError}
          </p>
        )}
        {successMsg && (
          <p style={{ margin: '8px 0 0', fontSize: '11px', color: 'rgba(134,239,172,0.8)' }}>
            ✓ {successMsg}
          </p>
        )}
      </div>
          ...
          {officialPhotos.length > 0 && (
            <div>
              <div style={{
                ...
                <span style={{ fontSize: '10px', color: textFaint }}>
                  · {officialPhotos.length} {officialPhotos.length === 1 ? 'photo' : 'photos'}
                </span>
              </div>
              <div style={{
                display:             'grid',
