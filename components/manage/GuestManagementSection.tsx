 ...
 *
 * Guest Management — inline component for Services tab
 * Covers: add guests, view access codes, RSVP tracking,
 *         check-in status, table assignment.
 *
 * Sub-sections:
    ...
    <div style={{ borderRadius: '10px', border: `1px solid ${isCheckedIn ? 'rgba(74,222,128,0.2)' : cardBorder}`, background: cardBg, marginBottom: '6px', overflow: 'hidden' }}>
      <div style={{ padding: '10px 14px', display: 'flex', alignItems: 'center', gap: '10px', cursor: 'pointer' }} onClick={() => setExpanded(e => !e)}>
        ...
        <div style={{ display: 'flex', gap: '6px', flexShrink: 0, alignItems: 'center' }}>
          {isCheckedIn && (
            <span style={{ fontSize: '9px', padding: '2px 7px', borderRadius: '5px', background: 'rgba(74,222,128,0.08)', border: '1px solid rgba(74,222,128,0.2)', color: 'rgba(134,239,172,0.8)', fontWeight: 700 }}>✓ In</span>
          )}
      ...
      {expanded && (
        <div style={{ padding: '10px 14px 14px', borderTop: `1px solid rgba(255,255,255,0.04)` }}>
          ...
          {g.dietary_requirements && (
            <p style={{ fontSize: '11px', color: textFaint, marginBottom: '8px' }}>🍽 {g.dietary_requirements}</p>
          )}
          ...
          <div style={{ display: 'flex', gap: '6px' }}>
            ...
            <button onClick={() => onDelete(g.id)} style={{ fontSize: '10px', padding: '5px 12px', borderRadius: '6px', border: '1px solid rgba(248,113,113,0.2)', background: 'transparent', color: 'rgba(248,113,113,0.6)', cursor: 'pointer' }}>Remove</button>
          </div>
        </div>
      )}
    </div>
...
export default function GuestManagementSection({ capsuleId, tables, phases }: Props) {
  ...
  return (
    <div>
      <p style={{ fontSize: '12px', color: textFaint, lineHeight: 1.65, marginBottom: '14px' }}>
        Manage your guest list. Each guest receives a unique access code. Tap a guest to manage their table assignment, RSVP status, and check-in.
      </p>

      {loading ? (
        <p style={{ fontSize: '12px', color: textFaint, textAlign: 'center', padding: '16px 0' }}>Loading guests…</p>
      ) : (
        <>
          ...
          {guests.length > 0 && (
            <div style={{ display: 'flex', gap: '4px', marginBottom: '12px', flexWrap: 'wrap' as const }}>
              {['all', ...TIER_OPTIONS.filter(t => guests.some(g => g.tier === t))].map(tier => (
                <button key={tier} onClick={() => setFilter(tier)} style={{ fontSize: '10px', padding: '4px 10px', borderRadius: '6px', border: `1px solid ${filter === tier ? 'rgba(226,195,107,0.45)' : cardBorder}`, background: filter === tier ? goldFaint : 'transparent', color: filter === tier ? gold : textFaint, cursor: 'pointer' }}>
